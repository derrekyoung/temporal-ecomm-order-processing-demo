...
> **Status: work in progress.** Built so far: happy path, automatic retry,
> saga compensation, the durability (worker-kill) demo, and order status
> visibility. Still to come: cancellation and approval signals, and the
> 15-minute run sheet.
...
followed by the two compensation activities, and `— customer was never
charged` in the terminal.
...
5. **[src/activities.ts](src/activities.ts)** is where side effects live: the
   ...
   narrative line like `[payment] authorized $99.98 … funds held, not yet
   charged`. Completed activity results are recorded in history and are never
